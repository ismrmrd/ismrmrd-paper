...
fatrix2_test_fun(A, A.imask_array, A.omask_array, arg.check1)
...
if arg.test_block
	fatrix2_test_block(A, arg.x, arg.complex, o1d, arg.tol_gram) % [,] [;] etc.
end
...
% fatrix2_test_fun()
function fatrix2_test_fun(A, imask, omask, check1)
...
if check1 % mtimes with "1" as a partial check of adjoint
	a1 = A * ones(np,1);
	r1 = ones(1,nd) * A;
	equivs(sum(a1), sum(r1), 'thresh', 5e-6)

	b1 = A' * ones(nd,1);
	c1 = ones(1,np) * A';
	equivs(sum(b1), sum(c1), 'thresh', 5e-6)
end
...
% gram: A'A
% kronI: kron(I,A)
function fatrix2_test_block(A, x, do_complex, o1d, tol_gram)
