...
%	smap = smap(:, :, [ncoil 1:(ncoil-1)]); % phase of last coil is cooler
	...
	im(6, mask4 .* angle(smap), [-pi pi], '<s'), cbar
...
%	sig = 0; % noiseless
	...
	im(7, abs(ykj), '|y|'), cbar
	im(8, ir_unwrap(angle(ykj)), [-pi pi], '< y'), cbar
end
...
%	im(4, angle(xhat), [-pi pi], '< x hat'), cbar
	im(3, real(xhat), 'Re(x hat)'), cbar
	im(4, imag(xhat), 'Im(x hat)'), cbar
	im(9, mask4 .* abs(shat), '|s hat|'), cbar
	im(10, mask4 .* angle(shat), [-pi pi], '< s hat'), cbar
