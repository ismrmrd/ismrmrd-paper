...
%|
%| options
%|	bodycoil [(N)]		reference image (optional) (default:
...
tmp = size(ykj);
NN = tmp(1:end-1); % todo: what about single surface coil case?
ncoil = tmp(end);
coildim = numel(tmp);

% default reference image
if isempty(arg.bodycoil)
	arg.bodycoil = sqrt(sum(abs(ykj).^2, coildim)); % sqrt-sum-of-squares
