...
if 1 % test transforms of each object type
	...
	for ii=1:nrow(tests)
		...
		if 0 % test vs old mri_objects_2d()
			if streq(otype, 'gauss2') || streq(otype, 'rect2')
				[xo yo] = mri_objects_2d(otype(1:end-1), param);
				equivs(i2, xo(ig.xg, ig.yg))
				equivs(f2, yo(fg{:}))
			end
		end
	end
...
if 1 % test transforms of each object type
	...
	for ii=1:nrow(tests)
		...
		if 0 % test vs old mri_objects_3d()
			if streq(otype, 'gauss3') || streq(otype, 'rect3')
				[xo yo] = mri_objects_3d(otype(1:end-1), param);
				equivs(i3, xo(ig.xg, ig.yg, ig.zg), 'thresh', 2e-6)
				equivs(f3, yo(fg{:}), 'thresh', 3e-6)
			end
		end
	end
prompt
