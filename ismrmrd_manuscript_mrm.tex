...
\ifdefined\isresponse
        \newcommand{\remark}[1]{{\marginpar{\bf \color{black} #1}}}
...
Sebastian~Kozerke$^9$,
Hui~Xue$^7$,
Adrienne~E.~Campbell-Washburn$^7$,
...
\item The ``encodingLimits" section indicates the ky center (5) and the minimum and maximum ky values (0 and 11 respectively), i.e., this is a partial Fourier experiment, with a true pixel size that is somewhat larger than the nominal resolution.
\end{itemize}
...
Each acquisition data header depicted in Fig.~\ref{fig:cstruct}, contains a set of encoding counters: \\*
\texttt{kspace\_encoding\_step\_1}, \texttt{slice}, etc. For a particular data set, the meaning of the encoding counters depends on the encoding space defined in the data set's XML header.  For the case of cartesian imaging, the mapping between ISMRMRD encoding counters and vendor specific dimensions is shown in Table~\ref{tab:counters}. The encoding counters also include a set of 8 integers and 8 floating point numbers that can be used \remark{R2.2}\added{in} an application specific way, for example they could refer to b-value and gradient direction in a diffusion weighted imaging sequence (see below). Or one could use them to label data from a spectroscopy or spectroscopic imaging experiment. Unfortunately, one of the natural counters for spectroscopy (\texttt{kspace\_encoding\_step\_0}) was omitted from version 1 of the acquisition data header. \remark{R2.3}\added{The missing \texttt{kspace\_encoding\_step\_0} would be specifically useful for 3D chemical shift imaging (CSI) a.k.a. 3D spectroscopic imaging.} This oversight will be corrected in version 2.
...
\item{Build system.} The C and C++ implementation use the CMake (Kitware \url{https://cmake.org}) build configuration system to support multiple operating systems (Linux, Microsoft Windows, Apple OS X).
...
The code repositories also contain the source code for the documentation (which use the Doxygen (\url{http://www.doxygen.org}) and Sphinx (\url{http://sphinx-doc.org}) projects) as well as example utility programs demonstrating how to use the APIs to read and write ISMRMRD files. The utility programs depend on the Boost library (\url{http://boost.org}) for command-line user interaction.
...
It is also worth emphasizing that the source code for the Siemens, Philips, and Bruker converters has been released publicly \remark{R1.1}\replaced{since it does not contain propritary files}{without approval from the respective vendors, and such approval is not required, as they contain no proprietary code}. The parts of the code that interact with the vendor raw data files, parse header and data structures etc, can be reverse engineered in a tedious but straightforward manner.  The source code for the GE converter \added{currently contains proprietary code and cannot be shared publicly, it} has been released through the GE collaborator network only. \deleted{because it includes proprietary code to interact with the vendor raw data files.  However, if one wished to create a fully public GE converter, it is possible to replace this functionality with a non-proprietary implementation via reverse engineering.} \added{The authors have had discussions with GE regarding a new converter that could be shared more freely, but at the time of writing this article, it has not been completed.}
...
\item Experimental data are collected and used to test/validate the prototype implementation.
...
\item The synthetic and experimental data sets are used to test/validate the high performance implementation of the algorithm.
\item The high-performance implementation of the algorithm is given approval from a local ethics board and put into production in a clinical environment.
\end{enumerate}
...
The raw data files were transferred off-line and converted into ISMRMRD using the vendor-specific converters described above. It was possible to convert the data from all four vendors. The resulting reconstructions are shown in Fig.~\ref{fig:demo}.
...
Figure~\ref{fig:epi} shows images reconstructed from an accelerated EPI sequence.  Data from a fully encoded GRE sequence (not shown) were used to estimate the coil sensitivities.  Both data sets were stored in a single ISMRMRD file.
...
The raw data in vendor proprietary formats and in ISMRMRD format are being distributed via Zenodo (\url{http://dx.doi.org/10.5281/zenodo.33166}). Alternatively the raw data can be downloaded using the \texttt{code/do\_get\_data.sh} script, which is intended for Linux/Apple. Readers are encouraged to download the data and source code.
