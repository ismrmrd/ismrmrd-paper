...
\madded[R1.4]{The maximum number of acquisitions per data set is $\approx 4\times10^6$ acquisitions because the \texttt{scan\_counter} is stored as a 32 bit unsigned integer.  This should be sufficient for current scanner technology.  Each acquisition can contain $65535$ samples because the \texttt{number\_of\_samples} is stored as a 16 bit unsigned integer.  Both of these counters could be increased up to 64 bits, which is the current limit imposed by the the HDF5 file format on the number of elements (acquisitions) in a dataset.}
